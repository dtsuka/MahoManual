# AGENTS.md — MahoManual

CMS操作マニュアルをMarkdownで作成し、HTML/PDFで納品するローカルツール。
スクショへの注釈(丸数字・枠・罫線・矢印・テキスト)を画像に焼き込まず、**HTML/CSSオーバーレイ**として描画するのが設計の核。データはすべてプレーンテキスト(md + JSON + YAML)で、AI(Codex / MCP)からも人間(GUI)からも同じファイルを操作する。
